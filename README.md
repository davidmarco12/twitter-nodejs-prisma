...
- [x] Add Cursor Based Pagination to `GET api/post/by_user/:user_id` endpoint. (You can see how it works [here](./src/types/index.ts)).
...
- [x] Users do not currently have a profile picture. Integrate with AWS S3 to store user profile pictures. Careful! Do not receive images in your endpoints. Make use of S3 Pre-signed URLs. Update the UserDTO to include the profile image. You can use a public S3 bucket as it doesn't contain private data.
